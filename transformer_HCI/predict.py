...
if __name__ == '__main__':
    while True:
        tfcv.logging.set_verbosity(tfcv.logging.ERROR)

        #arg_length = len(sys.argv)

        sick_input=input('::')
        arg_length = len(sick_input)
        if (arg_length < 2):
        ...
        '''
        input_word = ""
        for i in sick_input[0:]:
            input_word += i
            
        #print(type(input_word))
        predic_input_enc, predic_input_enc_length = data.enc_processing([input_word], char2idx)
        # 학습 과정이 아니므로 디코딩 입력은
